...
public class FIFOEvictionImpl<K> implements IEviction<K> {

    private Integer capacity;
    private Queue<K> keyTrack;

    public static class Builder<K> {
        public FIFOEvictionImpl<K> build() {
            keyTrack = new LinkedList<>();
            return new FIFOEvictionImpl<>(capacity, keyTrack);
        }
    }

    @Override
    public Optional<K> remove() {
        if (keyTrack.size() == capacity) {
